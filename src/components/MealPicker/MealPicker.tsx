...
const MealPicker = () => {
    const [activeMeal, setActiveMeal] = useState<MealKey>("breakfast"); //tracks which tab is selected
    const [searchTerm, setSearchTerm] = useState(""); //search bar thingy
    const [meals, setMeals] = useState<Record<MealKey, Food[]>>({
        breakfast: [],
        lunch: [],
        dinner: [],
        snacks: [],
    ...
    const filteredFoods = useMemo(() => {
        ...
        return ALL_FOODS.filter((food) => food.name.toLowerCase().includes(q));
    }, [searchTerm]);

    const handleAddFood = (food: Food) => {
        setMeals((currentMeals) => ({...currentMeals, [activeMeal]: [...currentMeals[activeMeal], food], })); //i allow duplicates
    };

    const handleRemoveFood = (mealKey: MealKey, index: number) => {
        setMeals((currentMeals) => {
        const updated = { ...currentMeals };
        updated[mealKey] = currentMeals[mealKey].filter((_, i) => i !== index);
        return updated;
        });
    };
    ...
    return (
        ...
        {/*left section - list of foods with a search bar*/}
        ...
        <div className={style.mealsSection}>
            ...
            {activeMealItems.length === 0 ? (
                ...
                activeMealItems.map((food, index) => (
                <div key={`${food.id}-${index}`} className={style.mealRow}>
                    <div className={style.mealText}>
                    <span className={style.mealName}>{food.name}</span>
                    ...
                    aria-label={`Remove ${food.name} from ${tabLabel[activeMeal]}`}
                    >
                    −
                    </button>
                </div>
