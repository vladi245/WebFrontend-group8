...
\end{center}
    ...
    \textbf{Authors:}\\[0.5cm]
Piotr Szczesny (Product Owner) – piszc24@student.sdu.dk \\
Vlad-Alexandru Ionita - vlion24@student.sdu.dk \\ 
Karolina Malik (Scrum Master) - kamal24@student.sdu.dk \\ 
...
Adjustable desks have become an important part of modern workplaces by reducing the negative effects of sitting too long and increasing the general health and productivity of employees. Many design trends have changed the office environment over the years, but recently integrating smart technologies such as IoT have become more popular. By connecting IoT technology with a smart desk, users can enjoy the convenience of pushing a button to achieve better ergonomics, health, and well-being while working at their desk. The project, in collaboration with LINAK, focuses on making the workspace smarter, healtier, and more productive. 

% ABSTRACT

\section*{Abstract}
In an office environment, while spending most of the days in front of a computer and a desk, people tend to forget about crucial breaks for their health. GetStanding addresses the growing need for healthier work environments by integrating reliable technology into adjustable desk systems provided by LINAK. 
The application allows users to input their body height which is used to calculate recommended sitting and standing desk height. Another key feature is the ability to control and monitor an adjustable desk.
It is done by connecting to the API attached to the desk and sending HTTP requests to modify the position of the desk. The project was created using a modern tech stack, with a React based frontend, while the backend was implemented using Node.js and includes the integration of a Raspberry Pi Pico board as an external edge gateway.
...
The team based on combined experience from previous projects and other courses, has decided to follow an Agile methodology during the development of GetStanding, supported by the use of Scrum roles.
\\\\
The development process was split into two-week sprints, with Jira used as a tool for organisation and keeping all team members on track. To ensure smooth development, each sprint had clear objectives that were decided during sprint planning meetings. Throughout those sessions, the backlog was populated with work items, which were assigned to adequate epics and split between team members.\\
...
For developing the frontend the team used React with Vite. Keeping the web application consistent across different pages was of great importance, therefore React, which is characterised by its component-based approach aligned with the teams vision. It made creating reusable user interface components possible, which made the development process easier by keeping the code clearer, easier to maintain and avoiding code repetition. Choosing Vite enabled the team to work in  a fast development environment with it's live development server, which provides feedback in real time.\\
...
Many studies have demonstrated that prolonged sedentary behaviour increases health risks such as obesity, cardiovascular diseases, as well as
decreased productivity. Issues such as back pain, shoulder stiffness, lower back pain, and mental health concerns have been noted as common reasons for decreased work performance. \\\\
A study~\cite{study} reports that people who predominantly sit at work have a 16\%  higher risk of mortality from all causes, and a 34\% higher risk of mortality from cardiovascular disease. To counteract the increased risk, sedentary individuals would have to engage in an additional 15 to 30 minutes of physical activity per day. \\\\
This project serves as a preventive health solution, aiming to encourage users to adopt healthier habits during their workday. GetStanding aims to track and monitor the duration of sitting and provide users insight into their standing habits over time. By sending reminder notifications, visualising weekly performance and tracking progress, GetStanding helps users stay motivated and consistent.

\subsection{Key Challenges} 
One of the primary challenges with this project is to encourage users to keep track of their daily time spent standing and adopt lasting behavioural changes. The process of habit formation requires sustained time and effort, many users may ignore the daily stats if they are too busy or if they lack motivation. This may decrease the effectiveness of the proposed solution. \\\\
...
\begin{itemize}
    \item Endpoints secured from unauthorised access.
    \item User can change their name.
    \item User can add and log their meals and workouts.
    \item User can see their own standing statistics.
\end{itemize}
...
\begin{itemize}
	...
	\item The system shall store and retrieve user information and desk preference data in a database.
	...
	\item The system shall retrieve and display the current desk height and position.
	 \item Users shall be able to customise their desk settings, such as preferred standing and sitting heights.
	\item The system shall provide periodic notifications regarding sitting time.
\end{itemize}
...
\begin{itemize}
    ...
    \item The notifications shall be non-invasive.
\end{itemize}
...
\section{Design}
This section outlines the design of GetStanding application, outlining the process of creating a software that fulfils the project requirements. This phase involved team efforts focused on defining system behaviour through scenarios and use cases, establishing user-interface design via prototyping and establishing the architecture and  timeline through different diagrams. Taking those steps has established a well-documented foundation for implementation and development of the system. \\\\
...
\begin{description}
    ...
    \item[\textbf{Context:}] The user is on the Desk Details page and wants to customise their position settings.
\end{description}
...
The development of the frontend involved a two-phase prototyping process. During the initial phase, team members created prototypes using Figma within a three day period. Following that deadline, the team reviewed them and selected the favoured design while identifying desirable elements from the alternative prototypes. These preferred elements were then integrated into the chosen design. Throughout the development process, minor details of the design were updated and refined to align with project objectives.\\\\ 
...
The sequence diagram provides an illustration of the system's flow and the interaction between its core modules. It details the process of adjusting the height of the desk. The proccess initiates when the application loads the desk page. Upon accomplishing a connection with the physical desk or the simulator, user actions such as dragging the height slider,setting their specific height preference or switching poisition of the desk trigger a request. This request is delivered to the backend which subsequently communicates with the desk or simulator using the dedicated Pico Gateway. This process results in the physical update of the desk's height to the value specified by the user.\\\\
...
The flowchart visually represents the process of user logging in and  the subsequent flow within the GetStanding Dashboard. The user's journey begins at the home page. If the user is new to the application then they are first required to register an account before proceeding to log in, while existing users navigate directly to log in process. Upon successful authentication the system directs the user to the appropriate dashboard either Premium or Standard based on their assigned account type. \\\\
...
\caption{Deployment Chart of GetStanding}
\end{figure}
...
\begin{figure}[H] 
	...
	\caption{Technology and Infrastructure of GetStanding}
\end{figure}
\newpage
...
\subsection{Desk Height Recommendation Algorithm}
% the formula 
\textbf{Sitting Height:}\\ 
H = ((0.198 * height) + 39.27)*10\\\\
\textbf{Standing Height:}\\
H = (0.6*height + 2.54)*10\\\\
\cite{recheightstand}, \cite{recheightsit}, \cite{ergonomics}, \cite{mayoclinic}, \cite{cornell}, \cite{uplift}, \cite{cand}
\subsection{Backend}
...
\begin{itemize}
	\item Raspberry Pi Pico acts as the External Edge Gateway, connecting with the GetStanding application.
...
The Raspberry Pi Pico was selected to handle communication between the GetStanding application, operating on an external network (GetStanding severs), and the internal system (desks in the office). Its role is to limit the connection with the outside world to be minimal and controlled. The gateway physically and logically restricts access by exposing only a narrowly defined interface and allowing only predefined data to be forwarded to internal endpoints for further validation.\\\\
...
Because the wifi2ble gateway resides entirely within the trusted internal network, it is intentionally not exposed to the external environment. Restriction of outside access is enforced at the network and router level, ensuring that only pre-approved internal traffic can reach the device gateway. This separation allows the system to use manufacturer-provided functionality while maintaining internal network isolation and security.
...
Available:\url{https://jamanetwork.com/journals/jamanetworkopen/fullarticle/2814094?}
2024.

\bibitem{recheightstand}
Kushagra Agarwal,"Standing Desk Height Calculator: Exact Settings for Your Height". Available:\url{https://lookaway.com/blog/2025/12/04/standing-desk-height-calculator-exact-settings-for-your-height/}. December 4th, 2025 
\bibitem{recheightsit}
Weberburo. "How do I optimally set up the height of my desk?", Available: \url{https://weberbuero.ch/en/blogs/magazine/optimally-set-up-desk-height?srsltid=AfmBOooQSWkMKOCxk5xiON8jlGTQjURZHNtLHJg0bsEVdwa2QWrcniYC}
\bibitem{ergonomics}
Kushagra Agarwal, "Work From Home Ergonomics: Your Ultimate Setup Checklist", Available: \url{https://lookaway.com/blog/2025/06/19/work-from-home-ergonomics-your-ultimate-setup-checklist/} June 19th, 2025 
\bibitem{mayoclinic}
Mayo Clinic Staff, "Office ergonomics: Your how-to guide", Available: \url{https://www.mayoclinic.org/healthy-lifestyle/adult-health/in-depth/office-ergonomics/art-20046169}
\bibitem{cornell}
Cornell University Ergonomics Web "Sitting and Standing at Work, Available: \url{https://ergo.human.cornell.edu/CUESitStand.html}
\bibitem{uplift}
UPLIFT Desk, "Ergonomic Calculator" , Available: \url{https://www.upliftdesk.com/ergonomic-calculator/}
\bibitem{cand}
Canadian Centre for Occupational Health and Safety, "Office Ergonomics - Working in a Standing Position", Available: \url{https://www.ccohs.ca/oshanswers/ergonomics/standing/standing_basic.html}
\end{thebibliography}
...
\begin{figure}[H]
    ...
    \caption{Manual test cases and results in TestRail}
    \label{fig:manual-tests}
\end{figure}

\captionsetup{font=footnotesize}
\section*{Code Review \& Conventions}
\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{conventions.jpg}\\
    \caption{Example of commits named according to Conventional Commits.}
\end{figure}

\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{conventions2.jpg}\\
    \caption{Example of code review for backend tests.}
\end{figure}

\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{PRreview1.jpg}\\
    \caption{Example of code review for early version of an admin.}
\end{figure}

\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{PRreview2.jpg}\\
    \caption{Example of code review with conversation about unused packages.}
\end{figure}

\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{PRreview3.jpg}\\
    \caption{Example of code review with a code example.}
\end{figure}

\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.9\textwidth]{PRreview4.jpg}\\
    \caption{Example of code review asking for files removal.}
\end{figure}
...
\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.4\textwidth]{allsprints.png}\\
    \caption{  All Sprint planning, review, and retrospective documentation through the project development.}
\end{figure}


\begin{figure}[H] 
    \centering 
    \includegraphics[width=0.5\textwidth]{sprint4.png}\\
    \caption{Sprint 4 planning document outlining sprint goals, team roles, and backlog items. }
\end{figure}

\begin{figure} [H]
	\centering
	\includegraphics[width=0.6\textwidth]{sprint3.png}
	\includegraphics[width=0.5\textwidth]{sprint3c.png}
    \caption{Sprint 3 review, including sprint metrics, burndown chart, completed backlog items, and team reflection. }
\end{figure}
...
\begin{figure} [H]
    ...
    \caption{Jira sprint backlog with completed tasks, task distribution, and collaboration. }
\end{figure}
